...
from chunkformer.utils.model_utils import get_output, get_output_with_timestamps, get_output_with_word_timestamps
...
class ChunkFormerModel(PreTrainedModel):
    ...
    def _load_audio_and_extract_features(self, audio_path):
        """
        ...
        if isinstance(audio_path, (str, os.PathLike)):
            ...
            audio = AudioSegment.from_file(BytesIO(audio_path))
    ...
    def endless_decode(
        ...
        """
        Perform streaming/endless decoding on short or long-form audio.
        ...
        # Calculate audio duration in seconds (assuming 10ms frame shift)
        fbank_conf = getattr(self.config, "fbank_conf", {})
        frame_shift = fbank_conf.get("frame_shift", 10)  # in milliseconds
        audio_duration = (xs_len * frame_shift) / 1000  # convert to seconds
        
        # Fast path for short audio (≤ 1 minute)
        if audio_duration <= 60:
            xs = xs.unsqueeze(0).to(device)
            xs_lens = torch.tensor([xs_len], dtype=torch.long, device=device)
            
            # Use forward_encoder with full attention (chunk_size=0 means full attention)
            encoder_out, encoder_mask = self.model.encoder.forward_encoder(
                xs=xs,
                xs_lens=xs_lens,
                chunk_size=0,  # 0 means full attention in forward_encoder
                left_context_size=0,
                right_context_size=0,
            )
            
            # Free input features immediately
            del xs, xs_lens

            # Decode based on model type
            if self.config.model == "asr_model":
                token_predictions = self.model.ctc.log_softmax(encoder_out).squeeze(0)
                token_predictions = torch.argmax(token_predictions, dim=-1).reshape(1, -1, 1)
            else:
                encoder_len = encoder_mask.squeeze(1).sum(-1)
                token_predictions = optimized_search(
                    self.model, encoder_out=encoder_out, encoder_out_lens=encoder_len
                )
                token_predictions = token_predictions.reshape(1, encoder_out.size(1), -1)
            
            # Free encoder output — no longer needed after search
            ...
            if self.char_dict is not None:
                if return_word_timestamps:
                    decode_result = get_output_with_word_timestamps(
                    ...
                    if not return_timestamps:
                        decode_result = " ".join([item["text"] for item in decode_result]).strip()
            else:
                decode_result = token_predictions

            # Free token predictions
            del token_predictions
            if device.type == "cuda":
                torch.cuda.empty_cache()

            return decode_result
        
        # Chunked processing for long audio (> 1 minute)
        ...
        # Free input features before concatenating encoder outputs
        del xs

        encoder_outs = torch.cat(encoder_outs, dim=1)  # [1, T, F]

        # Free caches — no longer needed
        del att_cache, cnn_cache
        if device.type == "cuda":
            torch.cuda.empty_cache()
        ...
        if self.config.model == "asr_model":
            ...
            encoder_len = torch.tensor(
                [encoder_outs.shape[1]], device=encoder_outs.device
            )
            token_predictions = optimized_search(
                self.model, encoder_out=encoder_outs, encoder_out_lens=encoder_len
            )
            token_predictions = token_predictions.reshape(
                1, encoder_outs.size(1), -1
            )

        # ── Free encoder outputs BEFORE post-processing ──────────────────
        del encoder_outs
        if device.type == "cuda":
            torch.cuda.empty_cache()
